...
const Main = () => {
  ...
  localStorage.setItem("category", JSON.stringify(selectedCategoriesArray));
  localStorage.setItem("subCategory",  JSON.stringify(selectedSubcategories));
  ...
  const handleSubcategoryClick = (subcategory, category) => {
    const isMultipleSelectAllowed = ["문화생활"].includes(category);
  ...
  return (
    <div style={{
      width: '100%',
      backgroundColor: '#fff',
      padding: '120px 100px', // padding을 설정해 내부 공간 조정
      boxShadow: 'none',
      textAlign: 'center',
    }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        {["밥", "카페", "팝업", "문화생활"].map((category) => (
          <div key={category} style={{ marginBottom: '40px', width: '60%' }}>
            <div style={{
              display: 'flex',
      ...
      <div style={{ marginTop: '80px' }}> {/* 데이트코스 생성 버튼 위쪽 간격 조정 */}
        <button
          ...
          onClick={() =>  moveToNextCategory({
            category: selectedCategoriesArray,
            navigate,
          })}
        >
          데이트 코스 만들기
